...
Here are several other data augmentation approaches for Jolly-Seber models. This first set is set up with individual survival covariates. Except for the marginalized formulation, these implementations use custom whole-history Gibbs samplers based on the block-update strategy of Wu et al. (2021). The Schwarz-Arnason implementations directly adapt the Type I and Type II recursions of Wu et al., whereas other formulations require model-specific whole-history updates to accommodate their different population or observation processes.. These samplers update entry, survival, departure, and, where applicable, superpopulation membership jointly rather than updating the latent states one at a time. The specific latent-state representation differs among models, and the Chandler and Clark (2014) formulation requires an adapted whole-history update because recruitment probabilities depend on realized abundance and therefore couple individual histories. The marginalized formulation integrates over the latent entry and survival states directly and therefore does not require a latent-state sampler. The whole-history Gibbs samplers improve efficiency, but massively so for the Chandler and Clark model.
