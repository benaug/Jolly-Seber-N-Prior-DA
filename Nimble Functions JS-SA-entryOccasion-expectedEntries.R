        ...
        if(model$z.super[i] == 0){
          ...
          if(e.curr < n.primary){
            for(g in (e.curr+1):n.primary){
              if(d.curr == g-1){
                if(rbinom(1,1,model$phi.int[i,g]) == 1){
                  d.curr <- g
                }
              }
            }
          ...
          if(n.primary > 1){
            ...
            for(k in 1:(n.primary-1)){
              g <- n.primary - k
              chi[g] <- q[g] * ((1 - model$phi.int[i,g+1]) +
                                  model$phi.int[i,g+1] * chi[g+1])
            }
          }
          ...
          if(e.curr < n.primary){
            for(g in (e.curr+1):n.primary){
              if(d.curr == g-1){
                ...
                surv.prob <- model$phi.int[i,g] * chi[g] /
                  ((1 - model$phi.int[i,g]) + model$phi.int[i,g] * chi[g])
