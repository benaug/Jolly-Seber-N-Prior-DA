...
This repository contains efficient MCMC samplers for Jolly–Seber models using **$N$-prior data augmentation**. Poisson priors are placed directly on population entry counts: the initial population size and the number of recruits entering before each subsequent primary occasion. This differs from Bernoulli data augmentation, including the restricted occupancy and Schwarz–Arnason formulations described by Royle and Dorazio (2008), in which the induced distributions of the entry counts converge to Poisson distributions as $M$ tends to infinity. In the restricted occupancy formulation, the binomial distributions for recruitment conditional on the number of augmented individuals that have not previously entered converge to Poisson distributions. In the Schwarz–Arnason formulation, the binomial distribution for the superpopulation size converges to a Poisson distribution, and multinomial allocation of this Poisson superpopulation among entry occasions produces independent Poisson entry counts. However, in practice, $M$ is often kept as small as possible without constraining abundance to reduce computation time, but a smaller $M$ can cause the variances of the induced entry distributions to be  smaller than their corresponding Poisson variances, and they arbitrarily depend on $M$. Placing Poisson priors directly on entry counts avoids this approximation and can improve MCMC efficiency, particularly for when modeling recruitment as per capita and as a function of realized rather than expected abundance, where the only current option (I believe) is the per-capita recruitment parameterization of the restricted-occupancy model described by Chandler and Clark (2014). 
