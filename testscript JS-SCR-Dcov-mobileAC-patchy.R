...
#exclude lower values to make patchy state space
...
#3) z.super=1, (either z state) "gap jumper" to better jump across state space gaps. proposals are a fixed multiplier of
#sigma.move
...
#if you monitor s, you can investigate ability of s sampler to jump state space gaps
#compare s posteriors to truth. Can s posterior get to true s cell?
mvSamples2 <- as.matrix(Cmcmc$mvSamples2)
mvSamples2 <- mvSamples2[-c(1:100),]
y2D <- apply(y.nim,c(1,2),sum)
