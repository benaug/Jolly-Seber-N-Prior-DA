...
Here are several other data augmentation approaches for Jolly-Seber models. This first set is set up with individual survival covariates. Except for the marginalized formulation, these implementations use custom whole-history Gibbs samplers based on the block-update strategy of Wu et al. (2021). The Schwarz-Arnason implementations directly adapt the Type I and Type II recursions of Wu et al., whereas other formulations require model-specific whole-history updates to accommodate their different population processes. These samplers update entry, survival, departure, and, where applicable, superpopulation membership jointly rather than updating latent states one at a time. The resulting updates are hard coded for the observation models used in these implementations: changing an observation model, or adding an additional observation process, generally requires modifying the whole-history sampler so that the appropriate observation likelihood contributions are included in the state-history update. The specific latent-state representation also differs among models, and the Chandler and Clark (2014) formulation requires an adapted whole-history update because recruitment probabilities depend on realized abundance and therefore couple individual histories. The marginalized formulation integrates over the latent entry and survival states directly and therefore does not require a latent-state sampler. The whole-history Gibbs samplers improve computational efficiency, with especially large gains for the Chandler and Clark model.
