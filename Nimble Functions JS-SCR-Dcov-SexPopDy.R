  ...
  run = function(){
    ...
    for(up in 1:z.super.ups){ #how many updates per iteration?
      ...
      if(updown==0){#subtract
        ...
        if(non.init>0){
          ...
          pick.idx <- seq(pick,M*n.primary,M) #used to reference correct y and pd nodes
          ...
          model$calculate(pd.nodes[pick.idx]) #update pd nodes when z.super/z changes
        ...
        if(noff.init>0){
          ...
          model$calculate(ER.M.nodes)
          model$calculate(ER.F.nodes)
          model$calculate(pd.nodes[pick.idx]) #update pd nodes when z.super/z/sex changes
          #get proposed logprobs for N and y
          lp.proposed.N.M <- model$calculate(N.M.nodes[1])
          lp.proposed.N.F <- model$calculate(N.F.nodes[1])
          lp.proposed.N.recruit.M <- model$calculate(N.recruit.M.nodes)
    ...
    for(i in 1:length(sex.up)){
      if(z.obs[sex.up[i]]==1){ #only do detected guys here
        i.idx <- seq(sex.up[i],M*n.primary,M) #used to reference correct y and pd nodes
