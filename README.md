...
Here are several other data augmentation approaches for Jolly-Seber models. This first set is set up with individual survival covariates. Except for the marginalized formulation, these implementations use custom block Gibbs samplers based on the Type I and Type II recursions of Wu et al. (2021), which build on the blocked latent-state updating approach of Dupuis and Schwarz (2007). Rather than updating latent states one occasion at a time or explicitly evaluating every possible latent history, the Type I and Type II recursions propagate the relevant conditional likelihoods through time and update the entry and departure portions of an individual's latent history as separate blocks. The Schwarz-Arnason implementations directly adapt the recursions of Wu et al., whereas other formulations require model-specific modifications to accommodate their different population processes. Where applicable, superpopulation membership is also updated using the corresponding marginalized block probabilities. The resulting updates are hard coded for the observation models used in these implementations: changing an observation model, or adding an additional observation process, generally requires modifying the block sampler so that the appropriate observation-likelihood contributions are included in the recursions. The specific latent-state representation also differs among models, and the Chandler and Clark (2014) formulation requires an adapted Type I/Type II update because recruitment probabilities depend on realized abundance and therefore couple individual histories. The marginalized formulation integrates over the latent entry and survival states directly and therefore does not require a latent-state sampler. These recursive block Gibbs samplers improve computational efficiency, with especially large gains for the Chandler and Clark model.
