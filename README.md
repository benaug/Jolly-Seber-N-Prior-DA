...
This repository also has various existing versions of Jolly-Seber models fit using data augmentation using the whole-history block-update approach of Wu et al. (2021). I will switch the N-prior data augmentation samplers to use this approach, which is more efficient that what I came up with.
...
Here are several other data augmentation approaches for Jolly-Seber models. This first set is set up with individual survival covariates. Except for the marginalized formulation, all models use custom whole-history Gibbs samplers based on the block-update approach of Wu et al. (2021). These samplers update entry, survival, departure, and, where applicable, superpopulation membership jointly rather than updating the latent states one at a time. The specific latent-state representation differs among models, and the Chandler and Clark (2014) formulation requires an adapted whole-history update because recruitment probabilities depend on realized abundance and therefore couple individual histories. The marginalized formulation integrates over the latent entry and survival states directly and therefore does not require a latent-state sampler. The whole-history Gibbs samplers improve efficiency, but massively so for the Chandler and Clark model.
