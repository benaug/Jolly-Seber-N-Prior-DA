...
Here are several other data augmentation approaches for Jolly-Seber models. This first set is set up with individual survival covariates. Except for the marginalized formulation, these implementations use custom block Gibbs samplers based on the Type I and Type II updating strategy of Wu et al. (2021). The Schwarz-Arnason implementations directly adapt the Type I and Type II recursions of Wu et al., whereas other formulations require model-specific adaptations to accommodate their different population processes. Rather than updating latent states one occasion at a time, these samplers update the entry portion and departure portion of an individual's latent history as separate blocks and, where applicable, also update superpopulation membership. The resulting updates are hard coded for the observation models used in these implementations: changing an observation model, or adding an additional observation process, generally requires modifying the block sampler so that the appropriate observation-likelihood contributions are included. The specific latent-state representation differs among models, and the Chandler and Clark (2014) formulation requires an adapted Type I/Type II update because recruitment probabilities depend on realized abundance and therefore couple individual histories. The marginalized formulation integrates over the latent entry and survival states directly and therefore does not require a latent-state sampler. These block Gibbs samplers improve computational efficiency, with especially large gains for the Chandler and Clark model.
