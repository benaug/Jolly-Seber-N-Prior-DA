    ...
    for(up in 1:z.super.ups){ #how many updates per iteration?
      ...
      if(updown==0){#subtract
        ...
        if(non.init>0){
          ...
          #   }
          # }
          # The reverse s proposal is exactly the s trajectory prior, so its proposal density cancels
          # the s prior density and does not need to be evaluated.
        ...
        if(model$N.super[1] < M){ #cannot update if z.super maxed out. Need to raise M
          ...
          if(noff.init>0){
            ...
            #simulate new s trajectory from its model prior; its proposal density is exactly its
            #model prior density, so the two terms cancel in the MH ratio.
            #OLD: #simulate new s trajectory and record forward proposal probs (prior and likelihood cancel, but including both below for clarity)
            ...
            for(g in 2:n.primary){
              model$s[pick,g,1:2] <<- rTruncNorm(1,s.prev=model$s[pick,g-1,1:2], sigma.move=model$sigma.move.int[g-1], xlim=xlim, ylim=ylim,z.super=1)
