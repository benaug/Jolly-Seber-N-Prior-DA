...
These models consider per capita recruitment as a function of expected N, rather than realized N as done in Chandler and Clark (2014) and the N-prior data augmentation models here. I prefer the use of realized N. For efficient MCMC, I removed the individual survival covariate from these implementations. Individual survival covariates can be included, but computing expected abundance then requires numerical integration, which can substantially slow the MCMC. In my comparisons, the sequential formulation mixes better because the entry-occasion formulation produces stronger posterior dependence between starting expected abundance and the per capita recruitment parameter. Both models take longer to compile than their analogues without per capita recruitment because of the additional dependencies, but run fast once compiled.

16. **JS-SA-sequential-perCapitaExpectedN**

    The demographic parameterization is similar to Link and Barker (2005): expected recruitment during an interval is proportional to expected abundance at the beginning of the interval. The expected numbers of entrants are converted to unconditional entry probabilities and then to the conditional entry probabilities used in the Royle and Dorazio (2008) sequential latent-state formulation. A custom Gibbs sampler updates complete latent trajectories using the block-update algorithm of Wu et al. (2021).

17. **JS-SA-entryOccasion-perCapitaExpectedN**

    The per capita recruitment parameterization is similar to Link and Barker (2005), where recruitment is defined relative to expected abundance, but the latent-state formulation remains the entry-occasion formulation described above. Expected recruitment is converted to unconditional entry probabilities, and each individual's entry occasion is represented directly as e[i] ~ dcat(pi[]), with survival modeled forward from entry. A custom Gibbs sampler updates superpopulation membership and complete latent trajectories using the block-update algorithm of Wu et al. (2021).