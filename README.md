...
12. **JS-CC**
    ...
    Nonspatial implementation of the Jolly–Seber approach of Chandler and Clark (2014), an extension of Restricted Occupancy that estimates per capita recruitment. The N-prior data augmentation models in this repository are exact Poisson specifications of this model.
   
13. **JS-SA-sequential**

    Nonspatial implementation of the Schwarz-Arnason Jolly–Seber approach of Royle and Dorazio (2008), included for comparison. This version uses the conditional entry probabilities and sequential latent-state parameterization of Royle and Dorazio. Rather than updating the latent states one occasion at a time, a custom Gibbs sampler updates each individual's complete entry, survival, and exit trajectory using the block-update algorithm of Wu et al. (2021), which is closely related to the latent-history block updates of Dupuis and Schwarz (2007). For undetected individuals, superpopulation membership is updated with the latent trajectory marginalized, followed by a draw of the complete trajectory conditional on the new membership state. This testscript and the next two are set up with the same parameter values and seed for comparison.
   
14. **JS-SA-entryOccasion**

    Nonspatial implementation of the Schwarz–Arnason Jolly–Seber approach of Royle and Dorazio (2008). This is the same model as JS-SA-sequential, but with a different latent-state parameterization. Instead of representing entry through conditional entry probabilities in a sequential latent-state model, entry occasion is represented directly as e[i] ~ dcat(pi[]), where pi has a Dirichlet prior, and survival is modeled forward from the entry occasion. A custom Gibbs sampler updates the complete entry, survival, and exit trajectory using the block-update algorithm of Wu et al. (2021). For undetected individuals, superpopulation membership is updated with the latent trajectory marginalized, followed by a draw of the complete trajectory conditional on the new membership state. Augmented individuals outside the superpopulation have their potential trajectories redrawn directly from their prior, which is also their full conditional. This can be viewed as a data-augmentation implementation of the original Schwarz and Arnason (1996) entry-time formulation. Related data-augmentation approaches that explicitly represent latent entry/birth and exit/death processes have been used by Schofield and Barker (2011), Matechou et al. (2016), and Wu et al. (2021).

15. **JS-SA-marginal**

    Nonspatial implementation of the Schwarz-Arnason Jolly–Seber approach of Royle and Dorazio (2008), included for comparison. Unlike the other Schwarz-Arnason implementations here, this version marginalizes the latent states out of the likelihood with a 3-state forward algorithm (not yet entered, alive, dead) rather than using the Wu et al. (2021) latent-trajectory Gibbs sampler. It retains the M augmented individuals to accommodate an individual survival covariate without numerical integration. N, B, and N.super are recovered each iteration by forward-filtering backward-sampling the trajectories from their full conditionals. Marginalizing the latent states prevents use of the conjugate updates available for some parameters in the explicitly augmented versions, and in the comparisons I have made this version appears less computationally efficient. Marginalization can be considerably faster when capture histories can be aggregated, for example when there are no individual random effects. Marginalization is not computationally feasible when recruitment is per capita as a function of realized abundance because individuals are then coupled through the population process. This model and marginalization approach have also been implemented in Stan, e.g., https://github.com/stan-dev/example-models/blob/master/BPA/Ch.10/js_super.stan, where marginalization is required because Stan cannot sample discrete parameters. HMC may be more efficient, but I have not made this comparison.

These models consider per capita recruitment as a function of expected N, rather than realized N as done in Chandler and Clark (2014) and the N-prior data augmentation models here. I prefer the use of realized N. For efficient MCMC, I removed the individual survival covariate from these implementations. Individual survival covariates can be included, but computing expected abundance then requires numerical integration, which can substantially slow the MCMC. The three models use the same relative demographic parameterization: expected recruitment during an interval is proportional to expected abundance at the beginning of the interval, and a separate superpopulation parameter supplies the absolute population scale. This parameterization is similar to Link and Barker (2005). The models differ in their latent-state representations. They take longer to compile than their analogues without per capita recruitment because of the additional dependencies, but run fast once compiled. In limited comparisons, RO is the fastest, but entry Occasion is not much less efficient. The SA formulation was least efficient.

16. **JS-RO-perCapitaExpectedN**

    Restricted-occupancy implementation of the per capita expected-abundance model. Relative expected recruitment is converted to unconditional entry probabilities, with a separate superpopulation parameter controlling the probability that an augmented individual ever enters during the study. These probabilities are then converted to the initial and conditional entry probabilities of the restricted-occupancy formulation. Unlike the Schwarz-Arnason implementations below, there is no separate superpopulation membership indicator: an augmented individual that is not in the superpopulation is represented directly by a trajectory that never enters. A custom Gibbs sampler updates each complete latent trajectory using the block-update algorithm of Wu et al. (2021), including a direct draw between never entering and the possible entry occasions for undetected individuals.

17. **JS-SA-sequential-perCapitaExpectedN**

    Schwarz-Arnason implementation using the sequential latent-state formulation of Royle and Dorazio (2008). Relative expected recruitment is converted to unconditional entry probabilities and then to the conditional entry probabilities used in the sequential state process. A separate superpopulation membership indicator determines whether each potential trajectory contributes to the realized population. A custom Gibbs sampler updates superpopulation membership and complete latent trajectories using the block-update algorithm of Wu et al. (2021).

18. **JS-SA-entryOccasion-perCapitaExpectedN**

    Schwarz-Arnason implementation using the entry-occasion latent-state formulation described above. Relative expected recruitment is converted to unconditional entry probabilities, and each potential individual's entry occasion is represented directly as e[i] ~ dcat(pi[]), with survival modeled forward from entry. A separate superpopulation membership indicator determines whether the potential trajectory contributes to the realized population. A custom Gibbs sampler updates superpopulation membership and complete entry, survival, and exit trajectories using the block-update algorithm of Wu et al. (2021).