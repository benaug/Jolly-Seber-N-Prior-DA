...
These models consider per capita recruitment as a function of expected N, rather than realized N as assumed in Chandler and Clark (2014) and the N-prior data augmentation models here (I prefer the use of realized N conceptually). For efficient MCMC, I removed the individual survival covariate from these implementations. Individual survival covariates can be included, but computing expected abundance then requires numerical integration, which can substantially slow down the MCMC. The three models use the same relative demographic parameterization: expected recruitment during an interval is proportional to expected abundance at the beginning of the interval, and a separate superpopulation-scale parameter supplies the absolute population scale. This parameterization is similar to Link and Barker (2005).
